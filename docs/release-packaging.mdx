...
description: "Reproducible local packaging steps for both desktop variants — full (World Monitor) and tech (Tech Monitor) — including signing and notarization."
---
...
The root install does not install `blog-site` dependencies. Those dependencies
are needed only for a full web build and are installed explicitly, with npm
lifecycle scripts disabled, by `npm run install:blog` or `npm run build:blog`.

Linux AppImage builds keep WebKit's sandbox enabled. As a last-resort diagnostic
for hosts where WebKit cannot start inside AppImage, setting
`WM_DISABLE_WEBKIT_SANDBOX=1` opts out with a security warning. Do not use that
override while viewing untrusted or remotely supplied content.

Desktop builds intentionally allow only bundled application scripts. Remote
JavaScript is blocked by the Tauri CSP, including the Clerk UMD bundles and the
YouTube iframe-control API. As a result, Clerk sign-in UI and scripted YouTube
player controls are unavailable in the hardened desktop build; ordinary remote
frames remain governed by `frame-src`. Do not re-enable remote `script-src`
hosts without treating them as privileged desktop code dependencies.
